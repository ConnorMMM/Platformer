...
namespace Platformer
{
    class Collision
    {
        ...
        public bool IsColliding(Sprite hero, Sprite otherSprite)
        {
            ...
            // It copares opposite edges of each rectangle, ie, the left edge of one with the right of the other
            if (hero.rightEdge < otherSprite.leftEdge || hero.leftEdge > otherSprite.rightEdge ||
                hero.bottomEdge < otherSprite.topEdge || hero.topEdge > otherSprite.bottomEdge)
            {
                // These tow rectangles are not colliding
                return false;
            }
        ...
        bool CheckForTile(Vector2 coordinates) // Checks if there is a tile at the specified coordinates
        {
            ...
            if (row < 0 || row > game.levelTileHeigth - 1)
            {
                return false;
            }
        ...
        public Sprite CollideWithPlatforms(Sprite hero, float deltaTime)
        {
            ...
            if (bottomCheck == true) // Check for collisions with the tiles below of the player
            {
